...
def generate_data(scale_factors):
    for format in formats:
        for factor in scale_factors:
            ...
            target = f"{s3_root}/{factor}/{format}"
            jobname=f"dsgen{factor}{format}"
            tag = spark_tags[0]
...
formats = ["parquet", "orc"]
#scale_factors = [2, 3]
generated = os.getenv("DATA_GENERATED")
